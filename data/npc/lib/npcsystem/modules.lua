	...
	function ShopModule:addBuyableItem(names, itemid, cost, itemSubType, attributes, realName)
		if SHOPMODULE_MODE ~= SHOPMODULE_MODE_TALK then
			...
			if shopItem == nil then
				self.npcHandler.shopItems[#self.npcHandler.shopItems + 1] = {id = itemid, buy = cost, sell = -1, subType = itemSubType, name = realName or ItemType(itemid):getName(), attributes = attributes, peso = modificarItensPeso[itemid] or 0}
			else
				shopItem.buy = cost
			end
		end
